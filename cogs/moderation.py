...
class ModerationCog(commands.Cog):
    ...
    async def p_setmodrole(self, ctx, role: discord.Role):
        ...
        if ctx.author.id != OWNER_ID and not await self.check_mod_role(ctx.author, ctx.guild.id):
    ...
    async def p_moderate(self, ctx, user: discord.Member):
        ...
        # Button callbacks (using the same logic as in slash commands)
        ...
        async def ban_callback(interaction):
            ...
            class BanModal(discord.ui.Modal, title="Ban User"):
                ...
                async def on_submit(self, interaction):
                    ...
                    try:
                        await user.ban(reason=f"Banned by {ctx.author} - {reason_text}", delete_message_days=delete_days_value)
                        result_embed = discord.Embed(
                            title="User Banned",
        ...
        async def timeout_callback(interaction):
            ...
            class TimeoutModal(discord.ui.Modal, title="Timeout User"):
                ...
                async def on_submit(self, interaction):
                    ...
                    try:
                        await user.timeout(timeout_duration, reason=f"Timed out by {ctx.author} - {reason_text}")
                        result_embed = discord.Embed(
                            title="User Timed Out",
